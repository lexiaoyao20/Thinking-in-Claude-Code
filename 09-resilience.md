...
Claude Code 在 `utils/errors.ts` 中定义了一套清晰的错误类型层级：
...
`withRetry.ts` 中的 `shouldRetry` 函数实现了一个精密的决策矩阵：
...
```typescript
// src/utils/config.ts (saveConfigWithLock 内部)
...
```typescript
// src/utils/config.ts (getConfig 内部)
