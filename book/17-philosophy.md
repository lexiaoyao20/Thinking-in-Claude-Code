# 第 17 章：编程哲学总结——从 Claude Code 中可迁移的设计模式
...
## 17.1 AsyncGenerator-as-Pipeline：用生成器编织多阶段流
...
## 17.2 Interface-Width-as-Control：宽接口给每个实现者留出差异化空间
...
**张力**：宽接口增加实现者的认知负担。Claude Code 用 `buildTool()` 的默认值（模式 17.5）来缓解——你只需要覆盖你关心的部分。

---

## 17.3 Permission-as-Spectrum：安全是光谱而非开关
...
## 17.4 Config-Priority-as-Trust：配置层映射信任层级
...
## 17.5 Fail-Closed-by-Default：`buildTool()` 的默认值哲学
...
## 17.7 Recursive-Self-Similarity：子 Agent 运行与父 Agent 相同的循环
...
## 17.8 Constraint-Driven-UI：终端约束下的 React 声明式模型
...
## 17.9 Context-Budget-Thinking：有限窗口中的最优信息留存
...
## 17.10 Deferred-Loading-for-Scale：运行时自适应的工具发现
...
## 17.11 Corruption-as-Normal：将损坏视为常态而非异常
...
## 17.12 Observable-Internals：从可观测到可控的运行时内省
...
graph LR
    subgraph "协同关系"
        A["17.5 Fail-Closed<br/>默认安全"] -->|"为宽接口提供安全网"| B["17.2 Wide Interface<br/>宽接口"]
        B -->|"差异化权限行为"| C["17.3 Permission Spectrum<br/>权限光谱"]
        D["17.9 Context Budget<br/>上下文预算"] -->|"驱动延迟加载需求"| E["17.10 Deferred Loading<br/>延迟加载"]
        F["17.7 Self-Similarity<br/>自相似性"] -->|"复用同一管道"| G["17.1 AsyncGenerator<br/>生成器管道"]
    end

    subgraph "张力关系"
        B -.->|"增加复杂度 vs"| H["17.5 Fail-Closed<br/>简单默认值"]
        E -.->|"冷启动延迟 vs"| I["17.12 Observable<br/>可观测性"]
        C -.->|"更多权限级别 vs"| J["17.4 Config Priority<br/>配置复杂度"]
        F -.->|"资源倍增 vs"| D
    end
...
**2. 宽接口 (17.2) vs Fail-Closed (17.5)**
...
quadrantChart
    ...
    "Fail-Closed (17.5)": [0.15, 0.85]
    "Corruption-Normal (17.11)": [0.25, 0.75]
    "File-as-IPC (17.6)": [0.2, 0.55]
    "Config-Priority (17.4)": [0.35, 0.7]
    "Permission-Spectrum (17.3)": [0.5, 0.8]
    "Observable (17.12)": [0.45, 0.65]
    "AsyncGen Pipeline (17.1)": [0.4, 0.6]
    "Context-Budget (17.9)": [0.6, 0.75]
    "Wide-Interface (17.2)": [0.5, 0.55]
    "Deferred-Loading (17.10)": [0.65, 0.6]
    "Self-Similarity (17.7)": [0.75, 0.85]
    "Constraint-UI (17.8)": [0.8, 0.5]
```
