...
`AsyncLocalStorage` 是 Node.js 提供的一个 API，允许在异步调用链中传递上下文数据而无需显式参数传递。Claude Code 用它来解决 In-Process Teammate 的身份隔离问题。

### 问题：同进程多身份

当多个 Teammate 在同一个 Node.js 进程中并发运行时，它们共享全局状态。如果 Teammate A 调用 `getAgentName()`，它需要得到 "researcher"，而不是同时运行的 Teammate B 的 "implementer"。
...
当启动一个 In-Process Teammate 时，整个执行链被包裹在 `runWithTeammateContext` 中：

```typescript
// 伪代码：In-Process Teammate 启动
runWithTeammateContext(teammateContext, () => {
  // 在这个闭包以及所有从它派生的异步操作中，
  // getTeammateContext() 都会返回 teammateContext
  await runAgent({ ... })
})
```
...
`teammate.ts` 中的辅助函数检查多个来源：

```
getAgentName() / getTeamName() / ...
  1. AsyncLocalStorage (TeammateContext)   ← In-Process Teammate
  2. dynamicTeamContext                    ← 运行时加入的 Teammate
  3. 环境变量 (CLAUDE_CODE_AGENT_NAME)     ← tmux/iTerm2 Teammate
```

这种分层查找确保了无论 Teammate 以何种方式运行，身份查询都能返回正确的值。

### 为什么不用参数传递？

你可能会问：为什么不把身份信息作为函数参数逐层传递？答案是**调用深度**。从 `runAgent` 到最终调用 `getAgentName()` 的地方，中间可能经过几十层函数调用。如果每一层都要传递一个 `context` 参数，不仅代码冗长，还容易在重构中遗漏。`AsyncLocalStorage` 就像"空气中的身份"——你呼吸的时候自然就有了，不需要有人递给你。
...
Leader                    Teammate
  │                         │
  │─── shutdown_request ──→ │  "你可以停了吗？"
  │                         │
  │                    （检查未保存的工作）
  │                         │
  │←── shutdown_approved ── │  "好的，我马上关"
  │        或                │
  │←── shutdown_rejected ── │  "不行，我还在忙"
  │                         │
```
...
AgentTool({ prompt: "Fix the null pointer in src/auth/validate.ts:42. The user field
